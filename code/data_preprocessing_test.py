...
y_train_test = y_train[0:3]

for i in range(len(X_train_test)):
    plt.imsave('./test_image/ori'+str(i)+'.png', X_train_test[i])

augmented_X_train, augmented_y_train = data_augmentation(X_train_test, y_train_test, data_augmentation_target)

for i in range(len(augmented_X_train)):
    plt.imsave('./test_image/aug'+str(i)+'.png', augmented_X_train[i])
...
for i in range(len(pre_X_train)):
    plt.imsave('./test_image/pre'+str(i)+'.png', pre_X_train[i, :, :, 0], cmap=cm.gray)
